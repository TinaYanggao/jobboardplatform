...
// Fetch messages received by this user (ignore self-sent messages)
$messages = [];
if (!empty($user_email)) {
    $stmt = $conn->prepare("
        SELECT m.id, m.message, m.job_id, m.sender_id,
               j.title AS job_title, u.full_name AS sender_name, u.email AS sender_email
        FROM messages m
        LEFT JOIN jobs j ON m.job_id = j.id
        LEFT JOIN users u ON m.sender_id = u.user_id
        WHERE m.receiver_email = ?  -- only messages sent to me
  ...
  <div class="mt-4">
    ...
    <?php if (!empty($messages)): ?>
        <?php foreach ($messages as $msg): ?>
            <div class="card mb-2">
                <div class="card-body">
                    <strong>From: <?php echo htmlspecialchars($msg['sender_name'] ?? $msg['sender_email']); ?></strong><br>
                    <?php if (!empty($msg['job_title'])): ?>
                        <small><em>Regarding Job: <?php echo htmlspecialchars($msg['job_title']); ?></em></small><br>
                    <?php endif; ?>
                    <p><?php echo nl2br(htmlspecialchars($msg['message'])); ?></p>

                    <!-- Reply form -->
                    <form method="POST" action="hire_talent.php" class="mt-2">
                        <input type="hidden" name="job_id" value="<?= $msg['job_id']; ?>">
                        <input type="hidden" name="receiver_email" value="<?= htmlspecialchars($msg['sender_email']); ?>">
