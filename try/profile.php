...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Keep current files if no new upload
    ...
    if (!empty($_FILES['profile_pic']['name'])) {
        ...
        if (move_uploaded_file($_FILES['profile_pic']['tmp_name'], $targetFile)) {
            $current_profile = $targetFile;
        }
    }

    // Update all fields at once
    $stmt = $conn->prepare("UPDATE users SET name=?, email=?, phone=?, address=?, skills=?, resume=?, profile_pic=? WHERE id=?");
...
.profile-header img { width:120px; height:120px; border-radius:50%; border:4px solid #032D84; object-fit:cover; }
.profile-header h2, .profile-header p { color:#032D84; }
.btn-primary { background-color:#032D84; color:#FEFEFE; border:2px solid #032D84; }
.btn-primary:hover { background-color:#ffffff; color:#032D84; }
.card { margin-bottom:20px; border:2px solid #032D84; border-radius:10px; }
.card-header { font-weight:bold; border-radius:10px 10px 0 0; background-color:#032D84; color:#FEFEFE; }
.modal-content { border-radius:10px; }
.modal-header { background-color:#032D84; color:#FEFEFE; }
.form-control { border-radius:5px; border:1px solid #032D84; }
.modal-footer .btn-primary { background-color:#032D84; color:#FEFEFE; border-color:#032D84; }
.modal-footer .btn-primary:hover { background-color:#ffffff; color:#032D84; border-color:#032D84; }
</style>
</head>
<body>
...
<img src="<?= !empty($profile_pic) ? htmlspecialchars($profile_pic) : 'https://via.placeholder.com/120'; ?>" alt="Profile Picture">
...
<div class="mb-3"><label>Upload Resume</label><input type="file" name="resume" class="form-control"><?php if($resume): ?><a href="<?= htmlspecialchars($resume); ?>" target="_blank" class="d-block mt-2">View Current Resume</a><?php endif; ?></div>
