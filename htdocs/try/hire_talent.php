...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['send_message'])) {
    ...
    if (!empty($message_text) && ($receiver_id > 0 || !empty($receiver_email))) {
        $stmt = $conn->prepare("INSERT INTO messages (job_id, sender_id, receiver_id, receiver_email, message, sent_at, status, created_at) VALUES (?, ?, ?, ?, ?, NOW(), 'unread', NOW())");
        $stmt->bind_param("iiiss", $job_id, $sender_id, $receiver_id, $receiver_email, $message_text);
        $stmt->execute();
        $stmt->close();
        header("Location: hire_talent.php");
        exit();
...
$sender_id = $_SESSION['user_id'];
$receiver_id = $applicant['user_id'] ?? 0;
$receiver_email = $applicant['applicant_email'];
...
$stmt2 = $conn->prepare("
    ...
    WHERE m.job_id = ?
      AND ((m.sender_id = ? AND (m.receiver_id = ? OR m.receiver_email = ?))
           OR (m.receiver_id = ? OR m.receiver_email = ?))
    ORDER BY m.created_at ASC
");
$stmt2->bind_param("iiiiis", $row['id'], $sender_id, $receiver_id, $receiver_email, $receiver_id, $receiver_email);
$stmt2->execute();
$messages_result = $stmt2->get_result();
?>
<div class="message-box">
<?php while ($msg = $messages_result->fetch_assoc()): ?>
<div class="<?= ($msg['sender_id'] == $_SESSION['user_id']) ? 'message-sent' : 'message-received'; ?>">
<small class="d-block mb-1"><?= htmlspecialchars($msg['sender_name'] ?? 'Applicant'); ?> (<?= date('M d, H:i', strtotime($msg['created_at'])) ?>)</small>
